...
public class RoomService
{
    ...
    public (bool Success, string? Error, Room? Room) JoinRoom(string roomCode, string connectionId, string playerId, string name)
    {
        lock (_lock)
        {
            ...
            var isReturningPlayer = room.Players.Any(p => p.PlayerId == playerId);
            if (room.Status != RoomStatus.WaitingForPlayers && !isReturningPlayer)
    ...
    public List<Room> GetAllInGameRooms()
    {
        return _rooms.Values.Where(r => r.Status == RoomStatus.InGame).ToList();
    }

    public void SetRoomStatus(string roomCode, RoomStatus status)
    {
        if (_rooms.TryGetValue(roomCode, out var room))
            room.Status = status;
    }
